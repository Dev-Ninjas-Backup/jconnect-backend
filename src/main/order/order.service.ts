...
export class OrdersService {
    ...
    /**
     * Seller didn't move the order to IN_PROGRESS within the 12h acceptDeadline
     * (set at order creation — see createOrderWithPaymentMethod()).
     */
    async autoCancelUnacceptedOrder(orderId: string) {
        return this.autoCancelExpiredOrder(orderId, OrderStatus.PENDING, {
            ...
            buyerBody: (order) =>
                `@${order.seller?.username ?? "The seller"} didn't accept order ${order.orderCode} within 12 hours. Your payment has been refunded.`,
            sellerBody: (order) =>
                `You missed the 12-hour window to accept order ${order.orderCode}. It has been cancelled and refunded to the buyer.`,
            emailBody: (order) =>
                `The seller did not accept your order <strong>${order.orderCode}</strong> for the service <strong>${order.service.serviceName}</strong> within 12 hours, so it has been automatically cancelled and your payment has been refunded.`,
        });
    }
